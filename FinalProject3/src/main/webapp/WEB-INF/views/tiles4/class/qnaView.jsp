...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		// 질문글 삭제버튼
		$("button#qnaDelete").click(function(){
			...
			if(bool){
				...
				frm.method = "post";
		   	    frm.action = "<%= ctxPath%>/class/qnaDeleteEnd.sam";
		   	    frm.submit();
			}
		});
...
<c:if test="${requestScope.qnavo.depthno eq 0 && sessionScope.loginuser.identity eq 1}"> 
	...
	<span style="color:red; font-size: 10pt;">&nbsp;*답변 작성 후에는 수정은 가능하나 삭제가 불가합니다.</span>
				...
				<table id="table" style="width:100%; border-top: 1.5px #b3b3b3 solid; border-bottom: 1.5px #b3b3b3 solid;">
			       <tr>
					   ...
					   <td style="text-align: center;">
					     <input style="width:100%;" name="subject" id="subject"/>
					   </td>
					</tr>
			       <tr>
					   <th style="width:200px;">내용</th>
					   <td style="text-align: center;">
					   	 <textarea rows="10" cols="100" style="width: 100%; height: 100px;" name="content" id="content"></textarea>
					    				     
					   </td>
					</tr>				
				</table>
				<input type="hidden" name="fk_perno" value="${sessionScope.loginuser.perno}" />
				<input type="hidden" name="groupno" value="${requestScope.qnavo.groupno}"/>
				<input type="hidden" name="fk_qnano" value="${requestScope.qnavo.qnano}"/>
				<input type="hidden" name="depthno" value="${requestScope.qnavo.depthno}"/>
				
				<div id="btn-board">
					<button type="button" class="btn-board" id="write" onclick="goAddAnswer()">답변하기</button>			
 	...
 	<form name="DelFrm">	
		...
		<input type="hidden" name="fk_qnano" value="${requestScope.qnavo.fk_qnano}"/>
	</form> 
